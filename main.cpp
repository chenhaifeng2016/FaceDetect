...
class HelloRequestHandler : public HTTPRequestHandler {
    void handleRequest(HTTPServerRequest &request, HTTPServerResponse &response) {
        ...
        std::vector<TrackFaceInfo> * trackFaceInfo = new std::vector<TrackFaceInfo>();

        int ret = api->track_max_face(trackFaceInfo, image.c_str(), 1);
        std::string result = "";
        ...
        if (ret == 0) {
            ...
            if (trackFaceInfo == NULL || trackFaceInfo->size() == 0) {
                nlohmann::json j;
                j["errno"] = 1;
                j["msg"] = "no face";
                result = j.dump();
            } else {
                TrackFaceInfo& faceInfo = trackFaceInfo->at(0);
...
int main(int argc, char **argv) {
    try {
        ...
        if (argc == 1) {
            ...
            std::cout << "FaceDetect port" << std::endl;
            std::cout << "For example" << std::endl;
            std::cout << "FaceDetect 8080" << std::endl;
            return 0;
        }
